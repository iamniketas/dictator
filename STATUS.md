...
| Вставка текста | ✅ | Unicode/Clipboard в зависимости от размера |
| История записей | ✅ | WAV + TXT + JSON в recordings/ |
...
├── history.rs        # История записей (WAV + текст)
...
recordings/           # Сохранённые записи (авто-создание)
└── YYYY-MM-DD/
    ├── {timestamp}.wav   # Аудио
    ├── {timestamp}.txt   # Текст
    └── {timestamp}.json  # Метаданные
```
...
## 2026-02-21 Update (v0.2.4)

### История записей (History)
- **Автоматическое сохранение:** Все аудио-записи и расшифровки сохраняются в `recordings/YYYY-MM-DD/`
- **Формат файлов:**
  - `{timestamp}.wav` — аудио (16kHz, mono, 16-bit PCM)
  - `{timestamp}.txt` — текст расшифровки
  - `{timestamp}.json` — метаданные (время, длительность, режим)
- **Автоочистка:** Файлы старше `retention_days` дней автоматически удаляются
- **Доступ из трея:**
  - **"Recent (click to copy):"** — список последних 10 записей с временем и превью
  - **"Open Recordings Folder"** — открывает папку с записями
  - Клик на запись в меню → текст копируется в буфер обмена
- **Git:** Папка `recordings/` добавлена в `.gitignore`

### Конфигурация истории
```toml
[history]
enabled = true         # Сохранять записи
retention_days = 7     # Хранить 7 дней
max_recent = 5         # Макс. записей в меню (future)
```

### Исправление: Потеря последних секунд в стриминге
- **Проблема:** Последние секунды аудио терялись при остановке стриминга
- **Причина:** Баг в логике обработки stop_signal с continue
- **Фикс:** Убран лишний continue, финальный буфер теперь обрабатывается корректно

### Исправление: Повреждение текста при вставке
- **Проблема:** Большие тексты (>500 символов) вставлялись медленно и повреждались
- **Причина:** SendInput имеет лимит на количество событий (~60-128)
- **Фикс:** 
  - Малые тексты (<500 символов): Unicode input по chunks
  - Большие тексты (>500 символов): Clipboard + Ctrl+V (быстро и надёжно)
