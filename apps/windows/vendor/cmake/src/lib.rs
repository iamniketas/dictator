...
impl Config {
    ...
    pub fn build(&mut self) -> PathBuf {
        ...
        let effective_generator_toolset = self
            .generator_toolset
            .clone()
            .or_else(|| env::var_os("CMAKE_GENERATOR_TOOLSET"));
        ...
        if target.contains("windows-gnu") {
            ...
            if !is_ninja && !using_nmake_generator {
                if target.contains("x86_64") {
                    if effective_generator_toolset.is_none() {
                        cmd.arg("-Thost=x64");
                    }
                    cmd.arg("-Ax64");
        ...
        if let Some(ref generator_toolset) = effective_generator_toolset {
